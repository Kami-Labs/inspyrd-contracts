...
pub mod inspyrd_machine {
    ...
    pub fn mint_nft<'info>(
        ...
        let inspyrd_machine = &mut ctx.accounts.inspyrd_machine;
        let inspyrd_machine_creator = &ctx.accounts.inspyrd_machine_creator;
        let clock = &ctx.accounts.clock;
        // Note this is the wallet of the Inspyrd machine
        ...
        let mut price = inspyrd_machine.data.price;
        if let Some(es) = &inspyrd_machine.data.end_settings {
            match es.end_setting_type {
                EndSettingType::Date => {
                    if clock.unix_timestamp > es.number as i64 {
                        if *ctx.accounts.payer.key != inspyrd_machine.authority {
                            return Err(ErrorCode::InspyrdMachineNotLive.into());
                        }
                    }
                }
                EndSettingType::Amount => {
                    if inspyrd_machine.items_redeemed >= es.number {
                        return Err(ErrorCode::InspyrdMachineNotLive.into());
                    }
                }
            }
        ...
        if let Some(gatekeeper) = &inspyrd_machine.data.gatekeeper {
            ...
            // verifies that the gatway token was not created before the inspyrd
            // machine go_live_date (avoids pre-solving the captcha)
            match inspyrd_machine.data.go_live_date {
                Some(val) => {
                    msg!(
                    ...
                    if (expire_time - EXPIRE_OFFSET) < val {
                        if let Some(ws) = &inspyrd_machine.data.whitelist_mint_settings {
            ...
            match assert_is_ata(whitelist_token_account, &payer.key(), &ws.mint) {
                Ok(wta) => {
                    if wta.amount > 0 {
                        ...
                        match inspyrd_machine.data.go_live_date {
                            None => {
                                if *ctx.accounts.payer.key != inspyrd_machine.authority && !ws.presale
                                {
                                    return Err(ErrorCode::InspyrdMachineNotLive.into());
                                }
                            }
                            Some(val) => {
                                if clock.unix_timestamp < val
                                    && *ctx.accounts.payer.key != inspyrd_machine.authority
                                    && !ws.presale
                                {
                                    return Err(ErrorCode::InspyrdMachineNotLive.into());
                                }
                        ...
                        assert_valid_go_live(payer, clock, inspyrd_machine)?;
                    }
                }
                Err(_) => {
        ...
        if inspyrd_machine.items_redeemed >= inspyrd_machine.data.items_available {
            return Err(ErrorCode::InspyrdMachineEmpty.into());
        }

        if let Some(mint) = inspyrd_machine.token_mint {
        ...
        let modded: usize = index
            .checked_rem(inspyrd_machine.data.items_available)
            .ok_or(ErrorCode::NumericalOverflowError)? as usize;

        let config_line = get_config_line(&inspyrd_machine, modded, inspyrd_machine.items_redeemed)?;

        inspyrd_machine.items_redeemed = inspyrd_machine
            .items_redeemed
            .checked_add(1)
            .ok_or(ErrorCode::NumericalOverflowError)?;

        let cm_key = inspyrd_machine.key();
        ...
        let mut creators: Vec<mpl_token_metadata::state::Creator> =
            vec![mpl_token_metadata::state::Creator {
                address: inspyrd_machine_creator.key(),
                verified: true,
                share: 0,
            }];

        for c in &inspyrd_machine.data.creators {
        ...
        let metadata_infos = vec![
            ...
            inspyrd_machine_creator.to_account_info(),
        ];
        ...
        invoke_signed(
            &create_metadata_accounts(
                ...
                *ctx.accounts.payer.key,
                inspyrd_machine_creator.key(),
                config_line.name,
                inspyrd_machine.data.symbol.clone(),
                config_line.uri,
                Some(creators),
                inspyrd_machine.data.seller_fee_basis_points,
                true,
                inspyrd_machine.data.is_mutable,
            ),
        ...
        invoke_signed(
            &create_master_edition(
                ...
                *ctx.accounts.metadata.key,
                *ctx.accounts.payer.key,
                Some(inspyrd_machine.data.max_supply),
            ),
            master_edition_infos.as_slice(),
            &[&authority_seeds],
        )?;

        let mut new_update_authority = Some(inspyrd_machine.authority);

        if !inspyrd_machine.data.retain_authority {
        ...
        for index in 0..num_instructions {
            ...
            if program_id != inspyrd_machine::id()
    ...
    pub fn update_inspyrd_machine(
        ctx: Context<UpdateInspyrdMachine>,
        data: InspyrdMachineData,
    ) -> ProgramResult {
        let inspyrd_machine = &mut ctx.accounts.inspyrd_machine;

        if data.items_available != inspyrd_machine.data.items_available
            && data.hidden_settings.is_none()
        {
            return Err(ErrorCode::CannotChangeNumberOfLines.into());
        }

        if inspyrd_machine.data.items_available > 0
            && inspyrd_machine.data.hidden_settings.is_none()
            && data.hidden_settings.is_some()
        {
            return Err(ErrorCode::CannotSwitchToHiddenSettings.into());
        }

        inspyrd_machine.wallet = ctx.accounts.wallet.key();
        inspyrd_machine.data = data;

        if ctx.remaining_accounts.len() > 0 {
            inspyrd_machine.token_mint = Some(ctx.remaining_accounts[0].key())
        } else {
            inspyrd_machine.token_mint = None;
        }
        Ok(())
    }
    ...
    pub fn add_config_lines(
        ...
        let inspyrd_machine = &mut ctx.accounts.inspyrd_machine;
        let account = inspyrd_machine.to_account_info();
        ...
        if index > (inspyrd_machine.data.items_available as u32) - 1 {
            return Err(ErrorCode::IndexGreaterThanLength.into());
        }
        if inspyrd_machine.data.hidden_settings.is_some() {
        ...
        let bit_mask_vec_start = CONFIG_ARRAY_START
            + 4
            + (inspyrd_machine.data.items_available as usize) * CONFIG_LINE_SIZE
            + 4;
    ...
    pub fn initialize_inspyrd_machine(
        ctx: Context<InitializeInspyrdMachine>,
        data: InspyrdMachineData,
    ) -> ProgramResult {
        let inspyrd_machine_account = &mut ctx.accounts.inspyrd_machine;

        if data.uuid.len() != 6 {
            return Err(ErrorCode::UuidMustBeExactly6Length.into());
        }

        let mut inspyrd_machine = InspyrdMachine {
            data,
        ...
        if ctx.remaining_accounts.len() > 0 {
            ...
            inspyrd_machine.token_mint = Some(*token_mint_info.key);
        }

        let mut array_of_zeroes = vec![];
        while array_of_zeroes.len() < MAX_SYMBOL_LENGTH - inspyrd_machine.data.symbol.len() {
            array_of_zeroes.push(0u8);
        }
        let new_symbol =
            inspyrd_machine.data.symbol.clone() + std::str::from_utf8(&array_of_zeroes).unwrap();
        inspyrd_machine.data.symbol = new_symbol;

        // - 1 because we are going to be a creator
        if inspyrd_machine.data.creators.len() > MAX_CREATOR_LIMIT - 1 {
            return Err(ErrorCode::TooManyCreators.into());
        }

        let mut new_data = InspyrdMachine::discriminator().try_to_vec().unwrap();
        new_data.append(&mut inspyrd_machine.try_to_vec().unwrap());
        let mut data = inspyrd_machine_account.data.borrow_mut();
    ...
    pub fn update_authority(
        ...
        if let Some(new_auth) = new_authority {
            inspyrd_machine.authority = new_auth;
        }
    ...
    pub fn withdraw_funds<'info>(ctx: Context<WithdrawFunds<'info>>) -> ProgramResult {
        ...
        let pay = &ctx.accounts.inspyrd_machine.to_account_info();
...
fn get_space_for_inspyrd(data: InspyrdMachineData) -> core::result::Result<usize, ProgramError> {
...
/// Create a new inspyrd machine.
#[derive(Accounts)]
#[instruction(data: InspyrdMachineData)]
pub struct InitializeInspyrdMachine<'info> {
    #[account(zero, rent_exempt = skip, constraint = inspyrd_machine.to_account_info().owner == program_id && inspyrd_machine.to_account_info().data_len() >= get_space_for_inspyrd(data)?)]
    inspyrd_machine: UncheckedAccount<'info>,
...
/// Add multiple config lines to the inspyrd machine.
#[derive(Accounts)]
pub struct AddConfigLines<'info> {
    #[account(mut, has_one = authority)]
    inspyrd_machine: Account<'info, InspyrdMachine>,
    authority: Signer<'info>,
}

/// Withdraw SOL from inspyrd machine account.
#[derive(Accounts)]
pub struct WithdrawFunds<'info> {
    #[account(mut, has_one = authority)]
    inspyrd_machine: Account<'info, InspyrdMachine>,
    #[account(address = inspyrd_machine.authority)]
    authority: Signer<'info>,
}
...
pub struct MintNFT<'info> {
    ...
    inspyrd_machine: Account<'info, InspyrdMachine>,
    #[account(seeds=[PREFIX.as_bytes(), inspyrd_machine.key().as_ref()], bump=creator_bump)]
    inspyrd_machine_creator: UncheckedAccount<'info>,
    payer: Signer<'info>,
    #[account(mut)]
    wallet: UncheckedAccount<'info>,
    ...
    // > Only needed if inspyrd machine has a gatekeeper
    // gateway_token
    // > Only needed if inspyrd machine has a gatekeeper and it has expire_on_use set to true:
    // gateway program
    // network_expire_feature
    // > Only needed if inspyrd machine has whitelist_mint_settings
    // whitelist_token_account
    // > Only needed if inspyrd machine has whitelist_mint_settings and mode is BurnEveryTime
    // whitelist_token_mint
    // whitelist_burn_authority
    // > Only needed if inspyrd machine has token mint
    // token_account_info
    // transfer_authority_info
}

/// Update the inspyrd machine state.
#[derive(Accounts)]
pub struct UpdateInspyrdMachine<'info> {
    #[account(
    mut,
    has_one = authority
    )]
...
/// Inspyrd machine state and config data.
#[account]
#[derive(Default)]
pub struct InspyrdMachine {
    pub authority: Pubkey,
    pub wallet: Pubkey,
    pub token_mint: Option<Pubkey>,
    pub items_redeemed: u64,
    pub data: InspyrdMachineData,
...
/// Inspyrd machine settings data.
#[derive(AnchorSerialize, AnchorDeserialize, Clone, Default)]
pub struct InspyrdMachineData {
    pub uuid: String,
    pub price: u64,
...
pub enum ErrorCode {
    ...
    #[msg("Can only provide up to 4 creators to inspyrd machine (because inspyrd machine is one)!")]
    TooManyCreators,
    ...
    TokenTransferFailed,
    #[msg("Inspyrd machine is empty!")]
    InspyrdMachineEmpty,
    #[msg("Inspyrd machine is not live!")]
    InspyrdMachineNotLive,
