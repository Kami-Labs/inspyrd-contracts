...
pub async fn setup_store(context: &mut ProgramTestContext) -> (Keypair, Keypair) {
    ...
    let accounts = mpl_price_sale_accounts::CreateStore {
    ...
    let data = mpl_price_sale_instruction::CreateStore {
        name: name.to_owned(),
...
pub async fn setup_selling_resource(
    ...
    let accounts = mpl_price_sale_accounts::InitSellingResource {
        store: store_keypair.pubkey(),
        admin: admin_wallet.pubkey(),
    ...
    let data = mpl_price_sale_instruction::InitSellingResource {
